function rois = fitRoisTo2dGaussians(img, rois, roiSearchBox, fitTolerance, searchInEllipse, ...
    visualize, uiProgressBarMessage)
%%
% Author: Marcel Goldschen-Ohm
% Email: marcel.goldschen@gmail.com
% 
% updated: 2019-12-03 DSW Included fitTolerance as argument
%
...
% fitTolerance:         Tolerance for fitting 2D gaussian. Default was
%                       1e-16.
...
if ~exist('uiProgressBarMessage','var')
    uiProgressBarMessage = 'Fitting ROIs to 2D Gaussians.';
end

%% Make sure ROIs have either a Center or Centroid field.
hasCenter = any(strcmp('Center', fieldnames(rois)));
if ~hasCenter
    hasCentroid = any(strcmp('Centroid', fieldnames(rois)));
    if ~hasCentroid
        errordlg('ERROR: findRoiGaussianCentersInImage requires ROIs with either a Center or Centroid field predefined.');
        return
    end
end
...
for i = 1:length(rois)
    ...
    if x0 >= 1 && x0 <= imageWidth && y0 >= 1 && y0 <= imageHeight
        ...
        warning('off','all');
        [fitresult,zfit,fiterr,zerr,resnorm,rr] = fmgaussfit(xx,yy,roiSubImage,fitTolerance);
        warning('on','all');
